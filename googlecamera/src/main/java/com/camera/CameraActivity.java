...
import com.camera.app.CameraAppUI;
import com.camera.app.CameraController;
import com.camera.app.CameraProvider;
import com.camera.app.CameraServices;
import com.camera.app.LocationManager;
import com.camera.app.MemoryManager;
import com.camera.app.MemoryQuery;
import com.camera.app.ModuleManager;
import com.camera.app.ModuleManagerImpl;
import com.camera.app.MotionManager;
import com.camera.app.OrientationManager;
import com.camera.app.OrientationManagerImpl;
import com.camera.app.eventprotos;
import com.camera.data.CameraDataAdapter;
import com.camera.data.FixedLastDataAdapter;
import com.camera.data.LocalData;
import com.camera.data.LocalDataAdapter;
import com.camera.data.LocalDataUtil;
import com.camera.data.LocalDataViewType;
import com.camera.data.LocalMediaData;
import com.camera.data.LocalMediaObserver;
import com.camera.data.LocalSessionData;
import com.camera.data.MediaDetails;
import com.camera.data.MetadataLoader;
import com.camera.data.PanoramaMetadataLoader;
import com.camera.data.RgbzMetadataLoader;
import com.camera.data.SimpleViewData;
import com.camera.debug.Log;
import com.camera.filmstrip.FilmstripContentPanel;
import com.camera.filmstrip.FilmstripController;
import com.camera.hardware.HardwareSpec;
import com.camera.hardware.HardwareSpecImpl;
import com.camera.module.ModuleController;
import com.camera.module.ModulesInfo;
import com.camera.one.OneCameraException;
import com.camera.one.OneCameraManager;
import com.camera.session.CaptureSession;
import com.camera.session.CaptureSessionManager;
import com.camera.settings.AppUpgrader;
import com.camera.settings.CameraSettingsActivity;
import com.camera.settings.Keys;
import com.camera.settings.SettingsManager;
import com.camera.settings.SettingsUtil;
import com.camera.tinyplanet.TinyPlanetFragment;
import com.camera.ui.AbstractTutorialOverlay;
import com.camera.ui.DetailsDialog;
import com.camera.ui.MainActivityLayout;
import com.camera.ui.ModeListView;
import com.camera.ui.PreviewStatusListener;
import com.camera.util.ApiHelper;
import com.camera.util.Callback;
import com.camera.util.CameraPerformanceTracker;
import com.camera.util.CameraUtil;
import com.camera.util.GalleryHelper;
import com.camera.util.GcamHelper;
import com.camera.util.GoogleHelpHelper;
import com.camera.util.IntentHelper;
import com.camera.util.PhotoSphereHelper;
import com.camera.util.QuickActivity;
import com.camera.util.ReleaseHelper;
import com.camera.util.UsageStatistics;
import com.camera.widget.FilmstripView;
import com.camera.widget.Preloader;
import com.qfl.camera2.portability.CameraAgent;
import com.qfl.camera2.portability.CameraAgentFactory;
import com.qfl.camera2.portability.CameraExceptionHandler;
import com.qfl.camera2.portability.CameraSettings;
...
public class CameraActivity extends QuickActivity
            ...
            new CameraAppUI.BottomPanel.Listener() {
                ...
                public void onEdit() {
                    ...
                    UsageStatistics.instance().mediaInteraction(fileNameFromDataID(currentDataId),
                            eventprotos.MediaInteraction.InteractionType.EDIT,
                            eventprotos.NavigationChange.InteractionCause.BUTTON,
                ...
                public void onDelete() {
                    final int currentDataId = getCurrentDataId();
                    UsageStatistics.instance().mediaInteraction(fileNameFromDataID(currentDataId),
                            eventprotos.MediaInteraction.InteractionType.DELETE,
                            eventprotos.NavigationChange.InteractionCause.BUTTON,
                            fileAgeFromDataID(currentDataId));
                ...
                public void onShare() {
                    ...
                    final int currentDataId = getCurrentDataId();
                    UsageStatistics.instance().mediaInteraction(fileNameFromDataID(currentDataId),
                            eventprotos.MediaInteraction.InteractionType.SHARE,
                            eventprotos.NavigationChange.InteractionCause.BUTTON,
                            fileAgeFromDataID(currentDataId));
            ...
            new FilmstripContentPanel.Listener() {
                ...
                public void onFilmstripHidden() {
                    ...
                    UsageStatistics.instance().changeScreen(currentUserInterfaceMode(),
                            eventprotos.NavigationChange.InteractionCause.SWIPE_RIGHT);
                ...
                public void onFilmstripShown() {
                    ...
                    UsageStatistics.instance().changeScreen(currentUserInterfaceMode(),
                            eventprotos.NavigationChange.InteractionCause.SWIPE_LEFT);
                ...
                public void onFocusedDataPromoted(int dataID) {
                    UsageStatistics.instance().mediaInteraction(fileNameFromDataID(dataID),
                            eventprotos.MediaInteraction.InteractionType.DELETE,
                            eventprotos.NavigationChange.InteractionCause.SWIPE_UP, fileAgeFromDataID(dataID));
                    removeData(dataID);
                }
                ...
                public void onFocusedDataDemoted(int dataID) {
                    UsageStatistics.instance().mediaInteraction(fileNameFromDataID(dataID),
                            eventprotos.MediaInteraction.InteractionType.DELETE,
                            eventprotos.NavigationChange.InteractionCause.SWIPE_DOWN,
    ...
    public void gotoGallery() {
        UsageStatistics.instance().changeScreen(eventprotos.NavigationChange.Mode.FILMSTRIP,
                eventprotos.NavigationChange.InteractionCause.BUTTON);
    ...
    public boolean onShareTargetSelected(ShareActionProvider shareActionProvider, Intent intent) {
        ...
        UsageStatistics.instance().mediaInteraction(fileNameFromDataID(currentDataId),
                eventprotos.MediaInteraction.InteractionType.SHARE,
                eventprotos.NavigationChange.InteractionCause.BUTTON, fileAgeFromDataID(currentDataId));
        // TODO add intent.getComponent().getPackageName()
        return true;
    }

    // Note: All callbacks come back on the main thread.
    private final CaptureSessionManager.SessionListener mSessionListener =
    ...
    public void onCreateTasks(Bundle state) {
        ...
        mModeListView.setVisibilityChangedListener(new ModeListView.ModeListVisibilityChangedListener() {
            @Override
        ...
        mPanoramaViewHelper = new PhotoSphereHelper.PanoramaViewHelper(this);
        ...
        if (!mSecureCamera) {
            ...
            v.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    UsageStatistics.instance().changeScreen(eventprotos.NavigationChange.Mode.GALLERY,
                            eventprotos.NavigationChange.InteractionCause.BUTTON);
                    startGallery();
                    finish();
                }
    ...
    public void onResumeTasks() {
        ...
        if (action == null) {
            source = eventprotos.ForegroundEvent.ForegroundSource.UNKNOWN_SOURCE;
        } else {
            switch (action) {
                case MediaStore.ACTION_IMAGE_CAPTURE:
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_IMAGE_CAPTURE;
                    break;
                case MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA:
                    // was UNKNOWN_SOURCE in Fishlake.
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_STILL_IMAGE_CAMERA;
                    break;
                case MediaStore.INTENT_ACTION_VIDEO_CAMERA:
                    // was UNKNOWN_SOURCE in Fishlake.
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_VIDEO_CAMERA;
                    break;
                case MediaStore.ACTION_VIDEO_CAPTURE:
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_VIDEO_CAPTURE;
                    break;
                case MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE:
                    // was ACTION_IMAGE_CAPTURE_SECURE in Fishlake.
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_STILL_IMAGE_CAMERA_SECURE;
                    break;
                case MediaStore.ACTION_IMAGE_CAPTURE_SECURE:
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_IMAGE_CAPTURE_SECURE;
                    break;
                case Intent.ACTION_MAIN:
                    source = eventprotos.ForegroundEvent.ForegroundSource.ACTION_MAIN;
                    break;
                default:
                    source = eventprotos.ForegroundEvent.ForegroundSource.UNKNOWN_SOURCE;
                    break;
    ...
    private int currentUserInterfaceMode() {
        int mode = eventprotos.NavigationChange.Mode.UNKNOWN_MODE;
        ...
        if (mFilmstripVisible) {
            mode = eventprotos.NavigationChange.Mode.FILMSTRIP;
        }
        return mode;
    }
    ...
    private static class PeekAnimationHandler extends Handler {
        private class DataAndCallback {
            LocalData mData;
            com.camera.util.Callback<Bitmap> mCallback;

            public DataAndCallback(LocalData data, com.camera.util.Callback<Bitmap>
                    callback) {
                mData = data;
                mCallback = callback;
            }
        ...
        public void startDecodingJob(final LocalData data,
                final com.camera.util.Callback<Bitmap> callback) {
    ...
    private void showDetailsDialog(int dataId) {
        ...
        UsageStatistics.instance().mediaInteraction(
                fileNameFromDataID(dataId), eventprotos.MediaInteraction.InteractionType.DETAILS,
                eventprotos.NavigationChange.InteractionCause.BUTTON, fileAgeFromDataID(dataId));
    }
