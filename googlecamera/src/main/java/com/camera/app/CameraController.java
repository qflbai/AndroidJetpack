...
import com.camera.CameraDisabledException;
...
import com.qfl.camera2.portability.CameraDeviceInfo.Characteristics;
/**
...
public class CameraController implements CameraAgent.CameraOpenCallback, CameraProvider {
     ...
     *                com.camera is disabled.
     * @param handler The {@link Handler} to post the com.camera
     *                callbacks to.
     * @param cameraManager Used for com.camera open/close.
     * @param cameraManagerNg Used for com.camera open/close with the new API. If
    ...
    public int getCurrentCameraId() {
        if (mCameraProxy != null) {
            ...
            Log.v(TAG, "getCurrentCameraId without an open com.camera... returning requested id");
    ...
    public void requestCamera(int id, boolean useNewApi) {
        ...
        // (T, F): Already requested the same com.camera. No-op. Return.
        ...
        if (mCameraProxy == null) {
            ...
            Log.v(TAG, "different com.camera already opened, closing then reopening");
            // Already has com.camera opened, and is switching cameras and/or APIs.
            if (mUsingNewApi) {
            ...
            // The same com.camera, just do a reconnect.
            Log.v(TAG, "reconnecting to use the existing com.camera");
    ...
    public void releaseCamera(int id) {
        if (mCameraProxy == null) {
            if (mRequestingCameraId == EMPTY_REQUEST) {
                ...
                Log.w(TAG, "Trying to release the com.camera before requesting");
            }
        ...
        if (mCameraProxy.getCameraId() != id) {
            throw new IllegalStateException("Trying to release an unopened com.camera.");
        }
        mRequestingCameraId = EMPTY_REQUEST;
    }
