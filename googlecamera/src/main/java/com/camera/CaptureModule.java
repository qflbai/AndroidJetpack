...
import com.camera.app.MediaSaver;
import com.camera.debug.DebugPropertyHelper;
...
import com.camera.one.Settings3A;
import com.camera.one.v2.OneCameraManagerImpl;
import com.camera.remote.RemoteCameraModule;
import com.camera.session.CaptureSession;
import com.camera.settings.Keys;
import com.camera.settings.SettingsManager;
import com.camera.ui.CountDownView;
import com.camera.ui.PreviewStatusListener;
import com.camera.ui.TouchCoordinate;
...
public class CaptureModule extends CameraModule
    ...
    /** Timeout for com.camera open/close operations. */
    ...
    /** The com.camera manager used to open cameras. */
    private OneCameraManager mCameraManager;
    /** The currently opened com.camera device, or null if the com.camera is closed. */
    private OneCamera mCamera;
    /** Held when opening or closing the com.camera. */
    private final Semaphore mCameraOpenCloseLock = new Semaphore(1);
    /** The direction the currently opened com.camera is facing to. */
    private OneCamera.Facing mCameraFacing = OneCamera.Facing.BACK;
    ...
    /** Handler thread for com.camera-related operations. */
    ...
    /** Current width of preview frames from com.camera. */
    private int mPreviewBufferWidth;
    /** Current height of preview frames from com.camera.. */
    ...
    public void resume() {
        ...
        // TODO: Check if we can really take a photo right now (memory, com.camera
        // state, ... ).
        ...
        // have to open the com.camera and start the preview here.
    ...
    public void onOrientationChanged(int orientation) {
        ...
        // the com.camera then point the com.camera to floor or sky, we still have
    ...
    public void onCameraAvailable(CameraAgent.CameraProxy cameraProxy) {
        // Ignore since we manage the com.camera ourselves until we remove this.
    }

    @Override
    public void hardResetSettings(SettingsManager settingsManager) {
        if (mStickyGcamCamera) {
            // Sitcky HDR+ mode should hard reset HDR+ to on, and com.camera back
            // facing.
    ...
    public void onSingleTapUp(View view, int x, int y) {
        ...
        if (mCameraFacing == OneCamera.Facing.FRONT) {
            return;
        }
    ...
    private void triggerFocusAtScreenCoord(int x, int y) {
        if (mCamera == null) {
            // If we receive this after the com.camera is closed, do nothing.
            return;
        }
        ...
        // Timeout in case com.camera fails to stop (unlikely).
        ...
        switch (state) {
            ...
            case PASSIVE_UNFOCUSED:
                mMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        mUI.setPassiveFocusSuccess(state == OneCamera.AutoFocusState.PASSIVE_FOCUSED);
                    }
                });
                break;
            case ACTIVE_FOCUSED:
            case ACTIVE_UNFOCUSED:
                if (!mTapToFocusWaitForActiveScan) {
                    mFocusedAtEnd = state != OneCamera.AutoFocusState.ACTIVE_UNFOCUSED;
                    mMainHandler.removeCallbacks(mHideAutoFocusTargetRunnable);
                    mMainHandler.post(mHideAutoFocusTargetRunnable);
                }
                break;
    ...
    private void measureAutoFocusScans(final OneCamera.AutoFocusState state, long frameNumber) {
        // Log AF scan lengths.
     ...
     *         returns the proper callback to be used for when the com.camera
        ...
        if (mStickyGcamCamera) {
            return new ButtonManager.ButtonCallback() {
                ...
                public void onStateChanged(int state) {
                    ...
                    // At the time this callback is fired, the com.camera id setting
                    // has changed to the desired com.camera.
                    SettingsManager settingsManager = mAppController.getSettingsManager();
                    if (Keys.isCameraBackFacing(settingsManager,
                            mAppController.getModuleScope())) {
                        throw new IllegalStateException(
                                "Hdr plus should never be switching from front facing com.camera.");
                    }

                    // Switch to photo mode, but request a return to hdr plus on
                    // switching to back com.camera again.
                    settingsManager.set(mAppController.getModuleScope(),
            ...
            return new ButtonManager.ButtonCallback() {
                ...
                public void onStateChanged(int cameraId) {
                    ...
                    // At the time this callback is fired, the com.camera id
                    // has be set to the desired com.camera.
                    mSettingsManager.set(mAppController.getModuleScope(), Keys.KEY_CAMERA_ID,
                            cameraId);

                    Log.d(TAG, "Start to switch com.camera. cameraId=" + cameraId);
    ...
    /**
     * Open com.camera and start the preview.
     */
    private void openCameraAndStartPreview() {
        // Only enable HDR on the back com.camera
        boolean useHdr = mHdrEnabled && mCameraFacing == OneCamera.Facing.BACK;
        ...
        try {
            ...
            if (!mCameraOpenCloseLock.tryAcquire(CAMERA_OPEN_CLOSE_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                throw new RuntimeException("Time out waiting to acquire com.camera-open lock.");
            }
        } catch (InterruptedException e) {
            throw new RuntimeException("Interrupted while waiting to acquire com.camera-open lock.", e);
        }
        if (mCamera != null) {
            // If the com.camera is already open, do nothing.
            Log.d(TAG, "Camera already open, not re-opening.");
            mCameraOpenCloseLock.release();
            return;
        }
        mCameraManager.open(mCameraFacing, useHdr, getPictureSizeFromSettings(),
                new OneCamera.OpenCallback() {
                    @Override
                    public void onFailure() {
                        Log.e(TAG, "Could not open com.camera.");
                        mCamera = null;
                        mCameraOpenCloseLock.release();
                        ...
                        camera.startPreview(new Surface(mPreviewTexture),
                                new OneCamera.CaptureReadyCallback() {
                                    ...
                                    public void onSetupFailed() {
                                        ...
                                        // in pause(), getting ready to close the com.camera.
                                        mCameraOpenCloseLock.release();
    ...
    /**
     * Re-initialize the com.camera if e.g. the HDR mode or facing property changed.
     */
    ...
    private Size getPictureSizeFromSettings() {
        String pictureSizeKey = mCameraFacing == OneCamera.Facing.FRONT ? Keys.KEY_PICTURE_SIZE_FRONT
    ...
    private int getPreviewOrientation(int deviceOrientationDegrees) {
        ...
        // orientation of the device (at least for the back-com.camera).

        // TODO: Remove this hack for the front com.camera as soon as b/16637957 is
        // fixed.
        if (mCameraFacing == OneCamera.Facing.FRONT) {
    ...
    /**
     * Returns which way around the com.camera is facing, based on it's ID.
     * <p>
