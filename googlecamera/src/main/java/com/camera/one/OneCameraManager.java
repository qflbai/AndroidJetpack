...
/**
 * The com.camera manager is responsible for instantiating {@link OneCamera}
 * instances.
 */
public abstract class OneCameraManager {
    ...
    /**
     * Attempts to open the com.camera facing the given direction with the given
     * capture size.
     *
     * Exactly one call will always be made to a single method in the provided
     * {@link OpenCallback}.
     *
     * @param facing which com.camera to open. The first com.camera found in the given
     *            direction will be opened.
     * @param enableHdr if an HDR feature exists, open a com.camera that supports it
     * @param captureSize the capture size. This must be one of the supported
     *            sizes.
     * @param callback this listener is called when the com.camera was opened or
     *            when it failed to open.
     * @param handler the handler on which callback methods are invoked.
     */
    public abstract void open(OneCamera.Facing facing, boolean enableHdr, Size captureSize,
                              OneCamera.OpenCallback callback, Handler handler);

    /**
     * Returns whether the device has a com.camera facing the given direction.
     */
    public abstract boolean hasCameraFacing(OneCamera.Facing facing);

    /**
     * Creates a com.camera manager that is based on Camera2 API, if available, or
     * otherwise uses the portability layer API.
     *
     * @throws OneCameraException Thrown if an error occurred while trying to
     *             access the com.camera.
     */
    ...
    /**
     * Creates a new com.camera manager that is based on Camera2 API, if available,
    ...
    private static OneCameraManager create(CameraActivity activity) throws OneCameraException {
        ...
        try {
            ...
            cameraManager = null;
            Log.e(TAG, "Could not get com.camera service v2", ex);
        }
        if (cameraManager != null && isCamera2Supported(cameraManager)) {
            ...
            return new com.camera.one.v2.OneCameraManagerImpl(
                    activity.getAndroidContext(), cameraManager, maxMemoryMB,
                    displayMetrics, activity.getSoundPlayer());
        } else {
            return new com.camera.one.v1.OneCameraManagerImpl();
        }
    }
    ...
    private static boolean isCamera2Supported(CameraManager cameraManager)
        ...
        try {
            ...
            Log.e(TAG, "Could not access com.camera to determine hardware-level API support.");
            return false;
        }
    }
