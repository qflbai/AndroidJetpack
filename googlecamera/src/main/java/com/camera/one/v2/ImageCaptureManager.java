...
import com.camera.util.ConcurrentSharedRingBuffer;
import com.camera.util.Task;
...
public class ImageCaptureManager extends CameraCaptureSession.CaptureCallback implements
    ...
    public static interface MetadataChangeListener {
         ...
         * will be called faster than the com.camera's frame rate.
         *
     ...
     * value, com.camera operations at the Java level have stalled, and are now
     ...
     *            one used for com.camera operations, such as capture requests and
    ...
    public void setCaptureReadyListener(final CaptureReadyListener listener) {
        mCapturedImageBuffer.setListener(mListenerHandler,
                new ConcurrentSharedRingBuffer.PinStateListener() {
                @Override
    ...
    public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
        ...
        // Detect com.camera thread stall.
        ...
        boolean swapSuccess = mCapturedImageBuffer.swapLeast(timestamp,
                new ConcurrentSharedRingBuffer.SwapTask<CapturedImage>() {
                @Override
    ...
    public void onImageAvailable(ImageReader reader) {
        ...
        if (img != null) {
            ...
            boolean swapSuccess = mCapturedImageBuffer.swapLeast(
                    img.getTimestamp(), new ConcurrentSharedRingBuffer.SwapTask<CapturedImage>() {
                            @Override
                        public CapturedImage create() {
                            CapturedImage image = new CapturedImage();
                            image.addImage(img);
