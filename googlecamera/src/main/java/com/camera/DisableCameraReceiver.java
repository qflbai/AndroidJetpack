...
// We want to disable com.camera-related activities if there is no com.camera. This
...
public class DisableCameraReceiver extends BroadcastReceiver {
    ...
    private static final String ACTIVITIES[] = {
        "com.android.com.camera.CameraLauncher",
    };

    @Override
    public void onReceive(Context context, Intent intent) {
        // Disable com.camera-related activities if there is no com.camera.
        boolean needCameraActivity = CHECK_BACK_CAMERA_ONLY
            ? hasBackCamera()
            : hasCamera();

        if (!needCameraActivity) {
            Log.i(TAG, "disable all com.camera activities");
        ...
        disableComponent(context, "com.android.com.camera.DisableCameraReceiver");
    }

    private boolean hasCamera() {
        int n = android.hardware.Camera.getNumberOfCameras();
        Log.i(TAG, "number of com.camera: " + n);
        return (n > 0);
    }
    ...
    private boolean hasBackCamera() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (info.facing == CameraInfo.CAMERA_FACING_BACK) {
                Log.i(TAG, "back com.camera found: " + i);
                return true;
            }
        }
        Log.i(TAG, "no back com.camera");
        return false;
    }
