 ...
 */
package com.qfl.camera2.other.blocking;
...
import com.qfl.camera2.other.exceptions.TimeoutRuntimeException;
...
public class BlockingCameraManager {
    ...
    public static class BlockingOpenException extends Exception {
        ...
        /**
         * Thrown when com.camera device enters error state during open, or if
         * it disconnects.
         *
         * @param errorCode
         * @param message
    ...
    /**
     * Create a new blocking com.camera manager.
     *
     * @param manager
    ...
    /**
     * Open the com.camera, blocking it until it succeeds or fails.
     *
     ...
     * highly unrecoverable, and all future calls to opening that com.camera will fail since the
     * service will think it's busy. This class will do its best to clean up eventually.</p>
     *
     * @param cameraId
     *            Id of the com.camera
     * @param listener
     *            Listener to the com.camera. onOpened, onDisconnected, onError need not be implemented.
     * @param handler
     ...
     *
     * <p>Time out after {@link #OPEN_TIME_OUT_MS} and unblock. Clean up com.camera if it arrives
     * later.</p>
     */
    private class OpenListener extends CameraDevice.StateCallback {
        ...
        public void onOpened(CameraDevice camera) {
            if (VERBOSE) {
                Log.v(TAG, "onOpened: com.camera " + ((camera != null) ? camera.getId() : "null"));
            }
        ...
        public void onDisconnected(CameraDevice camera) {
            if (VERBOSE) {
                Log.v(TAG, "onDisconnected: com.camera "
        ...
        public void onError(CameraDevice camera, int error) {
            if (VERBOSE) {
                Log.v(TAG, "onError: com.camera " + ((camera != null) ? camera.getId() : "null"));
            }
        ...
        CameraDevice blockUntilOpen() throws BlockingOpenException {
            ...
            if (!mDeviceReady.block(OPEN_TIME_OUT_MS)) {
                ...
                synchronized (mLock) {
                    if (mNoReply) { // Give the async com.camera a fighting chance (required)
                        mTimedOut = true; // Clean up com.camera if it ever arrives later
                        throw new TimeoutRuntimeException(String.format(
                                "Timed out after %d ms while trying to open com.camera device %s",
                ...
                if (mSuccess) {
                    ...
                    if (mDisconnected) {
                        throw new BlockingOpenException(
                                ...
                                "Failed to open com.camera device: it is disconnected");
                    } else if (mError != ERROR_UNINITIALIZED) {
                        throw new BlockingOpenException(
                                mError,
                                "Failed to open com.camera device: error code " + mError);
                    } else {
                        throw new AssertionError("Failed to open com.camera device (impl bug)");
                    }
                }
            }
        }
